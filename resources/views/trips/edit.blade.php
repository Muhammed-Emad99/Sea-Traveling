    ...
    <div class="main-content">
        <section class="section">
            ...
            <div class="section-body">
                ...
                <div class="row">
                    <div class="col-12">
                        <form action="{{ route('trips.update',$trip->id) }}" method="post" class="is-validated">
                            @csrf
                            @method('PUT')
                            <div class="form-group row mb-4">
                                <label class="col-form-label text-md-right col-12 col-md-3 col-lg-3">Trip Date</label>
                                <div class="col-sm-12 col-md-7">
                                    <input type="date" name="trip_date" value="{{ old('start_date',$trip->trip_date) }}" class="form-control @error('trip_date') is-invalid @enderror">
                                    @error('trip_date')
                                ...
                                <div class="col-sm-12 col-md-7">
                                    <select name="contract_id" class="form-control @error('contract_id') is-invalid @enderror">
                                        ...
                                        @foreach($contracts as $contract)
                                            <option value="{{ $contract->id }}" @selected(old('contract_id',$contract->id == $trip->contract_id ? $contract->id :''))>{{ $contract->contract_number }}</option>
                                        @endforeach
                                    </select>
                                    @error('client_id')
                                    <div class="invalid-feedback d-block">{{ $message }}</div>
